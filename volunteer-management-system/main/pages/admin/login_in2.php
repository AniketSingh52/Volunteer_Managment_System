...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($email) || empty($password)) {
        ...
        $query = "SELECT * FROM administration WHERE email = ?";
...
<body class="min-h-screen bg-gradient-to-br from-green-200 to-indigo-200 py-6 px-4">
    ...
    <div class="max-w-md mx-auto">
        ...
        <div class="text-center mb-8">
            ...
            <p class="mt-2 text-gray-600">Manage and Monitor the Activity</p>
        </div>

        <!-- Form -->
        <form id="loginForm" class="bg-white rounded-xl shadow-lg p-8 space-y-6" method="POST" action="login_in2.php">
            <?php if (isset($error)): ?>
            ...
            <!-- Submit Button -->
            <button type="submit"
                class="w-full bg-indigo-600 text-white py-2 px-4 rounded-lg hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 transition-colors">
                Sign In
            </button>
